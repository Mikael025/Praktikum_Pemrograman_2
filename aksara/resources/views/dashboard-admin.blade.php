<x-layouts.admin>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-md p-4 md:p-6 border border-slate-100">
            <h1 class="text-xl md:text-3xl font-bold text-slate-900">Selamat Datang, {{ auth()->user()->name ?? 'Admin' }}</h1>
            <p class="mt-2 text-sm md:text-base text-slate-600">Dashboard administrasi untuk mengelola kegiatan penelitian dan pengabdian masyarakat.</p>
        </div>
        ...
        <div class="space-y-3">
            ...
            <div class="bg-{{ $alert['type'] === 'warning' ? 'red' : 'blue' }}-50 border-l-4 border-{{ $alert['type'] === 'warning' ? 'red' : 'blue' }}-400 p-3 md:p-4 rounded-lg shadow">
                <div class="flex items-start gap-2 md:gap-3">
                    ...
                    <div class="flex-1">
                        <p class="text-xs md:text-sm font-medium text-{{ $alert['type'] === 'warning' ? 'red' : 'blue' }}-800">{{ $alert['message'] }}</p>
                        <p class="mt-1 text-xs md:text-sm text-{{ $alert['type'] === 'warning' ? 'red' : 'blue' }}-700">{{ $alert['action'] }}</p>
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-2 md:gap-3">
            <a href="{{ route('penelitian.index') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md hover:shadow-lg p-3 md:p-4 flex flex-col items-center justify-center transition-all transform hover:scale-105">
            ...
            <a href="{{ route('pengabdian.index') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md hover:shadow-lg p-3 md:p-4 flex flex-col items-center justify-center transition-all transform hover:scale-105">
                <svg class="w-6 md:w-8 h-6 md:h-8 mb-1 md:mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.informasi.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md hover:shadow-lg p-3 md:p-4 flex flex-col items-center justify-center transition-all transform hover:scale-105">
                <svg class="w-6 md:w-8 h-6 md:h-8 mb-1 md:mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"/>
                </svg>
                <span class="font-semibold text-xs text-center">Buat Berita</span>
            </a>
            <a href="{{ route('admin.laporan.index') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md hover:shadow-lg p-3 md:p-4 flex flex-col items-center justify-center transition-all transform hover:scale-105">
                <svg class="w-6 md:w-8 h-6 md:h-8 mb-1 md:mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                <span class="font-semibold text-xs text-center">Lihat Laporan</span>
            </a>
            <a href="{{ route('admin.laporan.perbandingan') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md hover:shadow-lg p-3 md:p-4 flex flex-col items-center justify-center transition-all transform hover:scale-105">
                <svg class="w-6 md:w-8 h-6 md:h-8 mb-1 md:mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
