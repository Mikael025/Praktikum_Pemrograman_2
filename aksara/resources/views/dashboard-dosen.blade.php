<x-layouts.dosen>
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-md p-6 border border-slate-100">
            <h1 class="text-2xl md:text-3xl font-bold text-slate-900">Selamat Datang, {{ auth()->user()->name ?? 'Dosen' }}</h1>
            <p class="mt-2 text-slate-600">Ringkasan aktivitas penelitian dan pengabdian Anda ditampilkan di bawah.</p>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <a href="{{ route('dosen.penelitian.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md hover:shadow-lg p-4 flex flex-col items-center justify-center transition-all transform hover:scale-105">
                <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                </svg>
                <span class="font-semibold text-sm">Penelitian Baru</span>
            </a>
            <a href="{{ route('dosen.pengabdian.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md hover:shadow-lg p-4 flex flex-col items-center justify-center transition-all transform hover:scale-105">
                <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                </svg>
                <span class="font-semibold text-sm">Pengabdian Baru</span>
            </a>
            <a href="{{ route('dosen.laporan.index') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md hover:shadow-lg p-4 flex flex-col items-center justify-center transition-all transform hover:scale-105">
                <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                <span class="font-semibold text-sm">Lihat Laporan</span>
            </a>
            <a href="{{ route('dosen.laporan.perbandingan') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md hover:shadow-lg p-4 flex flex-col items-center justify-center transition-all transform hover:scale-105">
                <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-gradient-to-br from-indigo-500 to-indigo-600 text-white rounded-2xl shadow-lg p-6 border border-indigo-400/20">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-indigo-100 text-sm font-medium">Total Kegiatan</p>
            ...
            <div class="bg-gradient-to-br from-slate-400 to-slate-500 text-white rounded-2xl shadow-lg p-6 border border-slate-300/20">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-100 text-sm font-medium">Tingkat Penyelesaian</p>
